...
export async function getTag(binderId: string, tagId: string): Promise<Tag | null> {
  const [tag] = await db
    .select()
    .from(tags)
    .where(and(eq(tags.id, tagId), eq(tags.binderId, binderId)));
  return tag ?? null;
}
...
export async function createTag(
  ...
  const [existing] = await db
    .select({ id: tags.id })
    .from(tags)
    .where(and(eq(tags.binderId, binderId), sql`LOWER(${tags.name}) = LOWER(${input.name.trim()})`))
    .limit(1);
  if (existing) throw new Error('A tag with this name already exists in this binder');

  const [tag] = await db
    .insert(tags)
    .values({ binderId, name: input.name.trim(), color: input.color ?? '#3B82F6' })
    .returning();
  return tag;
}

...
export async function listCategories(binderId: string): Promise<Category[]> {
  return db
    .select()
    .from(categories)
    .where(eq(categories.binderId, binderId))
...
export async function getCategory(binderId: string, categoryId: string): Promise<Category | null> {
  const [category] = await db
    .select()
    .from(categories)
    .where(and(eq(categories.id, categoryId), eq(categories.binderId, binderId)));
...
export async function createCategory(binderId: string, input: { name: string }): Promise<Category> {
  ...
  const [existing] = await db
    ...
    .where(
      and(
        eq(categories.binderId, binderId),
        sql`LOWER(${categories.name}) = LOWER(${input.name.trim()})`,
      ),
    )
    .limit(1);
  ...
  const [category] = await db
    .insert(categories)
    .values({ binderId, name: input.name.trim() })
    .returning();
  return category;
}

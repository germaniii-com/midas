...
export async function listAttachments(binderId: string, transactionId: string): Promise<Attachment[]> {
  ...
  }).from(transactionAttachments).where(eq(transactionAttachments.transactionId, transactionId)).orderBy(transactionAttachments.createdAt);
}

export async function getAttachment(transactionId: string, attachmentId: string): Promise<Attachment | null> {
  const [attachment] = await db.select().from(transactionAttachments).where(and(eq(transactionAttachments.id, attachmentId), eq(transactionAttachments.transactionId, transactionId))).limit(1);
...
export async function getAttachmentFile(attachmentId: string): Promise<{ buffer: Buffer; mimeType: string; fileName: string } | null> {
  const [attachment] = await db.select().from(transactionAttachments).where(eq(transactionAttachments.id, attachmentId)).limit(1);

...
export async function importFromActual(
  fileBuffer: Buffer,
  password: string,
  nameOverride?: string,
  currencyOverride?: string,
): Promise<{ id: string; name: string; description: string; currency: string }> {
  ...
  for (const n of noteRows) {
    ...
    if (content && content.trim()) {
      const m = String(n.id).match(/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i);
  ...
  lines.push('');
  lines.push(
    `INSERT INTO budget_binders (id, name, description, currency, password_hash, created_at) VALUES (${fmt(newBinderId)}, ${fmt(finalName)}, NULL, ${fmt(newCurrency)}, ${fmt(passwordHash)}, datetime('now'));`,
  );
  lines.push('');
  ...
  if (transactions.length > 0) {
    ...
    for (const tx of transactions) {
      const resolvedPayeeId =
        payeeMapping.get(tx.description as string) || (tx.description as string);
      ...
      if (!isTransfer) newPayeeId = payeeIdMap.get(resolvedPayeeId) || null;
  ...
  if (insertedTxIds.size > 0) {
    ...
    for (const tx of transactions) {
      ...
      if (cat && String(cat).trim()) {
        ...
        if (tagId) {
          ttRows.push({ binder_id: newBinderId, transaction_id: newTxId, tag_id: tagId });
        }
      }
    }
    const insert = buildInsert(
      'transaction_tags',
      ['binder_id', 'transaction_id', 'tag_id'],
      ttRows,
    );
    if (insert) lines.push(insert);
  }

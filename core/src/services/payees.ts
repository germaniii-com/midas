...
export async function getPayee(binderId: string, payeeId: string): Promise<Payee | null> {
  const [payee] = await db.select().from(payees).where(and(eq(payees.id, payeeId), eq(payees.binderId, binderId)));
  return payee ?? null;
}
...
export async function createPayee(binderId: string, input: { name: string }): Promise<Payee> {
  ...
  const [existing] = await db.select({ id: payees.id }).from(payees).where(and(eq(payees.binderId, binderId), sql`LOWER(${payees.name}) = LOWER(${input.name.trim()})`)).limit(1);
...
export async function deletePayee(binderId: string, payeeId: string): Promise<void> {
  const [payee] = await db.delete(payees).where(and(eq(payees.id, payeeId), eq(payees.binderId, binderId))).returning({ id: payees.id });
  if (!payee) throw new Error('Payee not found');
}

...
writeFileSync(
  ...
  JSON.stringify(
    { name: 'midas-backend', version: '1.0.0', private: true, type: 'commonjs' },
    null,
    2,
  ),
);
...
// Workspace packages are symlinked into node_modules (e.g. node_modules/@midas/core -> ../../core).
// They only need their build output (dist/) and package.json at runtime, not the source tree.
...
function copyWorkspacePackage(src, dest) {
  mkdirSync(dest, { recursive: true });
  const pkgSrc = resolve(src, 'package.json');
  if (existsSync(pkgSrc)) {
    cpSync(pkgSrc, resolve(dest, 'package.json'), { dereference: true });
  }
  const distSrc = resolve(src, 'dist');
  if (existsSync(distSrc)) {
    copyDir(distSrc, resolve(dest, 'dist'));
  }
}
...
function copyDep(name) {
  ...
  if (isSymlink(resolved)) {
    copyWorkspacePackage(resolved, target);
  } else {

...
- **Docker**: `backend/package.json` and `desktop/package.json` depend on `@midas/core` via `file:../core` (npm's `workspace:*` protocol throws `EUNSUPPORTEDPROTOCOL` for workspace→sibling-workspace deps in npm 10, so a file reference is used instead; npm links it as a symlink). `backend/Dockerfile` is built from the **repository root** context, installs only the `@midas/core` + `backend` workspaces (skipping root devDeps like Electron), builds core then backend, and forces sharp's platform binary (`npm install --no-save sharp`). The **prod** image contains only core's `package.json` + `dist/` (no source); the **dev** image keeps core + backend source for tsx hot reload. `docker-compose.yml` mounts `./backend` and `./core` into the container (masking their `node_modules`). A root `.dockerignore` excludes `node_modules`, `dist`, data dirs, etc.
